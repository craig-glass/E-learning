from django.urls import path
from . import views

app_name = "courses"

urlpatterns = [
    path('mine/',
         views.ManageCourseListView.as_view(),
         name='manage_course_list'),
    path('create/',
         views.CourseCreateView.as_view(),
         name='course_create'),
    path('<pk>/edit/',
         views.CourseUpdateView.as_view(),
         name='course_edit'),
    path('<pk>/delete/',
         views.CourseDeleteView.as_view(),
         name='course_delete'),
    path('<pk>/module/', views.CourseModuleUpdateView.as_view(),
         name='course_module_update'),
    path('module/<int:module_id>/content/<model_name>/create/',
         views.ContentCreateUpdateView.as_view(),
         name='module_content_create'),
    path('module/<int:module_id>/content/<model_name>/<id>/',
         views.ContentCreateUpdateView.as_view(),
         name='module_content_update'),
    path('content/<int:id>/delete/',
         views.ContentDeleteView.as_view(),
         name='module_content_delete'),
    path('question/<int:id>/delete/',
         views.QuestionDeleteView.as_view(),
         name='question_delete'),
    path('module/<int:module_id>/',
         views.ModuleListView.as_view(),
         name='module_list'),
    path('module/<int:module_id>/content/',
         views.ModuleContentListView.as_view(),
         name='module_content_list'),
    path('module/<int:module_id>/assignments/',
         views.AssignmentContentListView.as_view(),
         name='assignment_content_list'),
    path('module/<int:module_id>/assignments/create/',
         views.CourseAssignmentUpdateView.as_view(),
         name='course_assignment_update'),
    path('module/<int:module_id>/assignments/<int:assignment_id>/',
         views.AssignmentUpdateView.as_view(),
         name='assignment_update'),
    path('module/<int:module_id>/assignments/<int:assignment_id>/content/<model_name>/create/',
         views.AssignmentCreateUpdateView.as_view(),
         name='assignment_content_create'),
    path('module/<int:module_id>/quizzes/',
         views.QuizListView.as_view(),
         name='quiz_list_view'),
    path('module/<int:module_id>/quizzes/create/',
         views.QuizCreateView.as_view(),
         name='quiz_create'),
    path('module/<int:module_id>/quizzes/<int:quiz_id>/',
         views.QuizUpdateView.as_view(),
         name='quiz_edit'),
    path('module/<int:module_id>/quizzes/<int:quiz_id>/add_question/',
         views.QuizCreateUpdateView.as_view(),
         name='quiz_questions_create'),
    path('module/<int:module_id>/quizzes/<int:quiz_id>/<int:question_id>/add_choice/',
         views.AddChoiceView.as_view(),
         name='edit_choices'),
    path('module/order/', views.ModuleOrderView.as_view(),
         name='module_order'),
    path('content/order/', views.ContentOrderView.as_view(),
         name='content_order'),
    path('subject/<slug:subject>/',
         views.CourseListView.as_view(),
         name='course_list_subject'),
    path('all/',
         views.CourseListView.as_view(),
         name='course_list'),
    path('<slug:slug>/',
         views.CourseDetailView.as_view(),
         name='course_detail'),
]